...
pub fn sum(v: Vec<i32>) -> i32 {
    ...
    let half_index: usize = v.len() / 2;
    let halved_vector: (&[i32], &[i32]) = v.split_at(half_index);

    let first_half = halved_vector.0.to_vec();
    let second_half = halved_vector.1.to_vec();
