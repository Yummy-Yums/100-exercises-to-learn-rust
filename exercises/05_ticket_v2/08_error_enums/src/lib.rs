...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(error) => {
           match error{
               TicketNewError::TitleError(error) => panic!("Title error: {}", error),
               TicketNewError::DescriptionError(error) => Ticket {title, description: "Description not provided".into(), status}
           }
        }
    }
}
